...
export function Stepper({ steps, className }: LibraryStepperProps) {
  return (
    <ol className={[s.stepper, s.root, className].filter(Boolean).join(' ')} aria-label="Sonar waypoints">
      {steps.map((step) => {
        const itemCls =
          step.status === 'current' ? s.stepperItemCurrent : step.status === 'complete' ? s.stepperItemDone : s.stepperItem
        const ping =
          step.status === 'current' ? s.pingOn : step.status === 'complete' ? s.pingOff : s.ping
        return (
